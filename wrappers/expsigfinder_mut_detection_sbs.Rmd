...
knitr::opts_knit$set(root.dir = "/Users/ed4/Documents/phd/chemo_project/BotSeq/WP4/KCL_organoid_work/wp4_2530_20230401/extractions/wp4_2530_20230401_expsigfinder_SBS96/")
```
...
```{r read matrix}
mutation_matrix <- read.table("/Users/ed4/Documents/phd/chemo_project/BotSeq/WP4/KCL_organoid_work/wp4_2530_20230401/data/trint_subs_matrix_corrected.tsv", sep = "\t", header = T, as.is = T) 
```
...
burdens <- read.table("/Users/ed4/Documents/phd/chemo_project/BotSeq/WP4/KCL_organoid_work/wp4_2530_20230401/data/results.mut_burden_all.tsv", sep = "\t", header = T, as.is = T) 
```
...
bg_summary_stats <- metadata %>% filter(sample %in% control_names & patient==org) %>% dplyr::group_by(group) %>%
...
bg_CI_table <- as.data.table(filter(metadata, sample %in% control_names & patient == org))[,as.list(CI(burden)), by = group]
