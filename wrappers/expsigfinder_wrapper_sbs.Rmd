...
```{r read matrix}
mutation_matrix <- read.table("/Users/ed4/Documents/phd/chemo_project/BotSeq/WP4/KCL_organoid_work/wp4_2530_20230127/extractions/wp4_2530_20230324_expsigfinder_SBS96/sign_mutation_matrix_coverage_corrected.csv", sep = ",", header = T, as.is = T) 
```
...
bg_mean <- sum(PD41849_mutation_matrix$bg_profile)
print(bg_mean)
```
...
for (i in non_control_samples) {
  ...
  tryCatch({
    Wrap_KOSig(PD41849_mutation_matrix, "bg_profile", i, 100, bg_mean, 2, i)
    sig <- read.table(paste0(i,".txt"), sep = "\t", header = T, as.is = T)
    plotCountbasis(sig, 1, 6, 9, paste0(i, ".pdf"))
    plotPercentagebasis(sig, 1, 6, 9, paste0(i, "_percentage.pdf"))}, error=function(e){cat(paste0("ERROR for treatment group ", i, ": "),conditionMessage(e), "\n")})
}
...
determine whether the treatment spectrum is different from the control?
  - needs to incorporate the diffs in effective coverage somehow ..
