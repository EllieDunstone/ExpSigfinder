...
```{r select cluster}
cluster <- alk_subset_names
cluster_treatments <- c("Melphalan", "Bendamustine", "Chlorambucil", "Mechlorethamine")
```
...
#define custom subset
cluster_samples_subset <- cluster_samples[grep("IP", cluster_samples)]
...
colnames(final2_sigs)[ncol(final2_sigs)] <- "Activated_PhIP_sig" #depending on the ID of the merged sig, you may need to change the column name to include further info e.g. the organoid line
```
...
treatment_sig <- final_sigs$Melphalan_D95_sig
final2_sigs <- cbind(final2_sigs, treatment_sig)
colnames(final2_sigs)[ncol(final2_sigs)] <- "Melphalan_D95_sig"
```
