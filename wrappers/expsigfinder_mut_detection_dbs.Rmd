---
title: "ExpSigfinder mut detection - DBS"
...
Read in the DBS mutation matrix. This needs to be formatted with sample names in the "sample" column, and mutation classes as subsequent columns, with the counts of each mutation class in each sample as the matrix values.
...
Read in the corrected SBS mutation burden data for identification of samples showing significant increase compared to the background - this data is needed for calculation of the DBS burdens.
...
## Calculate the DBS burdens

We can now calculate the DBS burdens per base pair using the total number of DBS called per sample and the total base calls from the SBS burdens table.

```{r calculate dbs burdens}
#sum DBS to give total DBS calls
dbs_burdens <- cbind(colnames(mutation_matrix)[-1], colSums(mutation_matrix[,-1]))
colnames(dbs_burdens) <- c("sample", "dbs_call_sum")
burdens <- left_join(burdens, dbs_burdens, copy = TRUE)
burdens$dbs_call_sum <- as.numeric(burdens$dbs_call_sum)

#divide by total bases called to give indel burden per bp
burdens <- mutate(burdens, dbs_burden = dbs_call_sum / total)

#change NAs to zero
burdens[is.na(burdens)] <- 0
...
control_burdens_plot <- ggplot(controls, aes(reorder(sample_name, dbs_burden), dbs_burden, fill = treatment)) +
  geom_col() +
  geom_linerange(aes(x = sample_name, ymin = dbs_burden_lci, ymax = dbs_burden_uci)) +
  theme_classic() +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  xlab("Sample") +
  ylab("DBS burden per bp") + 
...
bg_summary_stats <- burdens %>% filter(sample %in% control_names & patient==org) %>% dplyr::group_by(group) %>%
   dplyr::summarise(mean = mean(dbs_burden), max = max(dbs_burden), min = min(dbs_burden), median = median(dbs_burden), sd = sd(dbs_burden), count = n())

#Add 95% CIs to bg burden summary stats
bg_CI_table <- as.data.table(filter(burdens, sample %in% control_names & patient == org))[,as.list(CI(dbs_burden)), by = group]
...
for (i in treatment_list) {
  control_v_treatment_burdens_plot <- ggplot(filter(mutation_matrix_metadata_burdens, patient == org & (group == "control" | treatment == i)), aes(reorder(sample_name, -dbs_burden), dbs_burden, fill = group)) + geom_col() +
  geom_linerange(aes(x = sample_name, ymin = dbs_burden_lci, ymax = dbs_burden_uci)) +
  theme_classic() +
...
mutagenesis_table <- select(mutation_matrix_metadata_burdens, sample, patient, tissue_sample, treatment, sample_name, concentration, tissue, group, total, dbs_burden, dbs_burden_lci, dbs_burden_uci)
...
mutagenesis_table <- mutagenesis_table %>% mutate(sign_mutagenesis = case_when(dbs_burden_lci >= bg_burden_uci ~ "Y", (dbs_burden_lci < bg_burden_uci) & (dbs_burden_uci >= bg_burden_lci) ~ "N", TRUE ~ "low" ))
...
write_csv(data_samples_of_interest, paste0(org, "_dbs_data_samples_of_interest.csv"))
write_csv(data_dropped_samples, paste0(org, "_dbs_data_dropped_samples.csv"))
write_csv(mutagenesis_table, paste0(org, "_dbs_mutagenesis_table.csv"))
```
