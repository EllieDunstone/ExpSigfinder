...
colnames(indel_burdens) <- c("sample", "indel_burden")
burdens <- left_join(burdens, indel_burdens, copy = TRUE)
```

We can also calculate the 95% confidence intervals for these burdens as follows:
...
Do we want to add any formal statistical test here? We have effectively the sample burdens, and upper and lower confidence intervals. For some pairs, the confidence intervals don't quite overlap, but there still may not be anything overly concerning going on?
